...
function AiPage({ snapshot, bridge }: { snapshot: Snapshot; bridge: EngineBridge }) {
  ...
  return <><PageHeader title="AI Coach" /><div className="ai-layout"><aside className="chat-list"><div className="chat-section-title chat-section-heading"><span>Chats</span><Button kind="ghost" size="sm" renderIcon={Add} onClick={() => bridge.dispatch("ai.newChat")}>New chat</Button></div>{snapshot.ai.chats.length === 0 && <p className="muted-copy">Start a chat to practice an argument.</p>}{snapshot.ai.chats.map((item) => <div className={`chat-row ${item.id === chat?.id ? "is-selected" : ""}`} key={item.id} onContextMenu={(event) => openChatMenu(event, item.id)}><button className="chat-item" onClick={() => bridge.dispatch("ai.selectChat", { id: item.id })}>{item.title}</button><OverflowMenu ariaLabel={`Actions for ${item.title}`}><OverflowMenuItem itemText="Rename" onClick={() => startRenameChat(item)} /><OverflowMenuItem itemText="Delete" isDelete onClick={() => setDeleteChat(item)} /></OverflowMenu></div>)}</aside><section className="chat-panel"><div className="chat-messages">{chat?.messages.length ? chat.messages.map((item, index) => <div className={`message ${item.role}`} key={`${item.timestamp}-${index}`}><span className="message-role">{item.role === "user" ? "You" : "AI Coach"}</span><p>{item.text}</p></div>) : <EmptyState icon={Chat} title="Start a coaching chat" description="Choose approved document context, then send a prompt." />}{snapshot.ai.loading && <InlineNotification kind="info" lowContrast title="AI Coach is thinking" hideCloseButton />}</div><div className="chat-composer"><TextArea id="ai-message" labelText="Message" hideLabel placeholder="Ask for a rebuttal drill..." value={message} onChange={(event) => setMessage(event.currentTarget.value)} rows={3} /><div className="chat-composer-actions"><MenuButton kind="ghost" size="sm" label={`Cite${cited.size ? ` (${cited.size})` : ""}`} menuAlignment="top-start" disabled={approvedDocuments.length === 0}>{approvedDocuments.map((doc) => <MenuItem key={doc.id} label={`${cited.has(doc.id) ? "✓ " : ""}${doc.name}`} onClick={() => bridge.dispatch("ai.toggleCitation", { id: doc.id, selected: !cited.has(doc.id) })} />)}</MenuButton><Button disabled={snapshot.ai.loading || !chat} onClick={() => { if (!message.trim()) return; bridge.dispatch("ai.sendMessage", { text: message.trim() }); setMessage(""); }}>Send</Button></div></div></section></div>{chatMenu && createPortal(<div className="document-context-menu" style={{ left: Math.max(8, Math.min(chatMenu.x, window.innerWidth - 248)), top: Math.max(8, Math.min(chatMenu.y, window.innerHeight - 160)) }} onMouseDown={(event) => event.stopPropagation()}><button type="button" onClick={() => { const item = snapshot.ai.chats.find((candidate) => candidate.id === chatMenu.id); if (item) startRenameChat(item); }}>Rename</button><button type="button" className="is-danger" onClick={() => { const item = snapshot.ai.chats.find((candidate) => candidate.id === chatMenu.id); setChatMenu(undefined); if (item) setDeleteChat(item); }}>Delete</button></div>, document.body)}<Modal open={Boolean(renameChat)} modalHeading="Rename chat" primaryButtonText="Rename" primaryButtonDisabled={!renameValue.trim()} secondaryButtonText="Cancel" onRequestClose={() => setRenameChat(undefined)} onRequestSubmit={() => { if (renameChat && renameValue.trim()) bridge.dispatch("ai.renameChat", { id: renameChat.id, title: renameValue.trim() }); setRenameChat(undefined); }}><TextInput id="rename-chat" labelText="Chat name" value={renameValue} onChange={(event) => setRenameValue(event.currentTarget.value)} autoFocus /></Modal><Modal open={Boolean(deleteChat)} danger modalHeading="Delete chat" primaryButtonText="Delete" secondaryButtonText="Cancel" onRequestClose={() => setDeleteChat(undefined)} onRequestSubmit={() => { if (deleteChat) bridge.dispatch("ai.deleteChat", { id: deleteChat.id }); setDeleteChat(undefined); }}><p>Delete {deleteChat?.title ?? "this chat"}?</p></Modal></>;
}
