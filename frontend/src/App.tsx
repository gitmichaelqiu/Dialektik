      ...
      {session.status !== "active" ? (
        ...
        <div className="round-grid">
          <section className="round-main">
            ...
            <Tile className="speech-order-tile"><div className="tile-kicker">Speech order</div><div className="speech-list">{session.speechOrder.map((speech, index) => <button className={`speech-row ${index === session.currentSpeechIndex ? "is-current" : ""}`} key={speech.id} disabled={!session.isHost} onClick={() => bridge.dispatch("session.selectSpeech", { index })}><span>{speech.label}</span><span>{Math.round(speech.durationMs / 60000)} min</span></button>)}</div></Tile>
          </section>
