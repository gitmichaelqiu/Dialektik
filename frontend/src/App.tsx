...
function WorkspaceContextMenu({ x, y, actions }: { x: number; y: number; actions: { label: string; danger?: boolean; onClick: () => void }[] }) {
  return createPortal(<div className="document-context-menu" style={{ left: Math.max(8, Math.min(x, window.innerWidth - 248)), top: Math.max(8, Math.min(y, window.innerHeight - actions.length * 44 - 16)) }} onMouseDown={(event) => event.stopPropagation()}>{actions.map((item) => <button key={item.label} type="button" className={item.danger ? "is-danger" : ""} onClick={item.onClick}>{item.label}</button>)}</div>, document.body);
}

function CiteMenu({ documents, cited, onToggle }: { documents: DebateDocument[]; cited: Set<string>; onToggle: (id: string, selected: boolean) => void }) {
  const [open, setOpen] = useState(false);
  ...
  return <><Button className="cite-menu-trigger" kind="ghost" size="sm" disabled={documents.length === 0} onClick={toggle}>Cite{cited.size ? ` (${cited.size})` : ""}</Button><CarbonMenu open={open} label="Cite" x={position.x} y={position.y} onClose={() => setOpen(false)}>{documents.length === 0 ? <MenuItem disabled label="No documents or evidence" /> : documents.map((document) => { const unavailable = !document.content.trim(); return <MenuItem key={document.id} disabled={unavailable} label={`${cited.has(document.id) ? "✓ " : ""}${document.name}${unavailable ? " (empty)" : ""}`} onClick={() => { onToggle(document.id, !cited.has(document.id)); setOpen(true); }} />; })}</CarbonMenu></>;
}
    ...
    <Theme theme={darkTheme ? "g100" : "white"}>
      <div className={`app-shell ${darkTheme ? "theme-dark" : ""} ${!isMobile && !navExpanded ? "nav-collapsed" : ""}`}>
        ...
        >
          ...
          <div className="sidebar-navigation"><SideNavItems>
            {navItems.map(({ page, label, icon: Icon }) => {
              ...
              return !isMobile && !navExpanded ? <CompactPreview key={page} label={label}>{link}</CompactPreview> : link;
            })}
          </SideNavItems></div>
          <div className="sidebar-documents-scroll"><DocumentSidebar compact={!isMobile && !navExpanded} snapshot={snapshot} bridge={bridge} selectedId={currentPage === "documents" ? selectedDocumentId : ""} onSelect={(id) => { setSelectedDocumentId(id); navigate("documents"); }} onCreate={() => { const id = `doc-${Date.now()}`; pendingDocumentId.current = id; setSelectedDocumentId(id); navigate("documents"); bridge.dispatch("document.create", { id, name: "Untitled note", folder: "private", mode: "write" }); }} onLink={() => setGoogleDialog({ open: true })} onEditGoogle={(document) => setGoogleDialog({ open: true, document })} /></div>
        ...
        <Content className="app-content">
          ...
          <div className="app-pagebar"><h1>{pageTitle}</h1><div className="pagebar-meta">{googleStatus && <span className="pagebar-status">{googleStatus}</span>}{currentPage === "inround" && snapshot.session?.status === "active" && <div className="pagebar-timers" aria-label="Live timers"><span className="pagebar-timer"><small>{snapshot.session.speechOrder[snapshot.session.currentSpeechIndex]?.label ?? "Speech"}</small><strong>{formatTimer(snapshot.session.speechRemainingMs)}</strong></span><span className="pagebar-timer pagebar-timer-secondary"><small>Prep</small><strong>{formatTimer(snapshot.session.prepRemainingMs)}</strong></span></div>}{currentPage === "inround" && snapshot.session && <Button kind="ghost" size="sm" onClick={() => bridge.dispatch("session.exit")}>Leave room</Button>}{currentPage === "documents" && selectedDocument?.externalUrl && <Button kind="ghost" size="sm" renderIcon={Launch} onClick={() => window.open(selectedDocument.externalUrl, "_blank", "noopener,noreferrer")}>Open in browser</Button>}</div></div>
...
function RoundPage({ snapshot, bridge, navigate }: { snapshot: Snapshot; bridge: EngineBridge; navigate: (page: Page) => void }) {
  ...
  const [winner, setWinner] = useState<"affirmative" | "negative">();
  ...
  return (
    <>
      <PageHeader eyebrow={session.status === "active" ? "Live round" : "Room lobby"} title={session.matchName || "Debate round"} description={`Room ${session.roomCode}${session.isHost ? " · You are hosting" : " · Connected"}`} />
      {session.status !== "active" ? (
        <Lobby session={session} snapshot={snapshot} bridge={bridge} />
      ) : (
        <div className="round-grid">
          <section className="round-main">
            <Tile className="timer-tile round-timer-card">
              ...
              <div className="timer-controls">
                <button type="button" className="timer-icon-button" aria-label={session.speechRunning ? "Pause timer" : "Start timer"} onClick={() => bridge.dispatch("timer.action", { timerType: "speech", action: session.speechRunning ? "pause" : "start" })}>{session.speechRunning ? <Pause /> : <Play />}</button>
                <Button kind="tertiary" renderIcon={Renew} onClick={() => bridge.dispatch("timer.action", { timerType: "speech", action: "reset" })}>Reset</Button>
              </div>
            </Tile>
            <Tile className="handout-tile"><div className="tile-kicker">Round handout</div><h2>{session.handout?.title || "No resolution yet"}</h2>{session.handout?.problem && session.handout.problem !== session.handout.title && <p className="round-resolution">{session.handout.problem}</p>}{session.handout?.details && session.handout.details !== session.handout.problem && <p className="muted-copy">{session.handout.details}</p>}</Tile>
            <SpeechOrder session={session} bridge={bridge} />
          </section>
          <aside className="round-aside"><Tile className="round-members-tile"><h3>Room members</h3>{session.debaters.length === 0 ? <p className="muted-copy">No other members yet.</p> : session.debaters.map((debater) => <div className="member-row" key={debater.id}><span>{debater.name}<small>{debater.team ? ` · ${debater.team}` : ""}</small></span><Tag type={debater.status === "connected" || debater.status === "approved" ? "green" : "gray"}>{debater.status}</Tag></div>)}</Tile><Tile className="prep-timer-tile"><h3>Prep timer</h3><div className="small-timer">{formatTimer(session.prepRemainingMs)}</div><div className="timer-actions"><Button kind="tertiary" onClick={() => bridge.dispatch("timer.action", { timerType: "prep", action: session.prepRunning ? "pause" : "start" })}>{session.prepRunning ? "Pause" : "Start"}</Button><Button kind="ghost" onClick={() => bridge.dispatch("timer.action", { timerType: "prep", action: "reset" })}>Reset</Button></div></Tile><Tile className="speaker-notes-tile"><h3>Speaker notes</h3><Select id="current-speaker" labelText="Speaker" value={selectedSpeaker} onChange={(event) => bridge.dispatch("session.selectSpeaker", { id: event.currentTarget.value })}>{session.debaters.map((debater) => <SelectItem key={debater.id} value={debater.id} text={debater.name} />)}</Select><TextArea id="speaker-notes" labelText="Private notes" value={speakerNotes[selectedSpeaker] ?? ""} onChange={(event) => bridge.dispatch("session.updateNotes", { speakerId: selectedSpeaker, text: event.currentTarget.value })} rows={4} disabled={!selectedSpeaker} /></Tile><CustomTimers session={session} bridge={bridge} /><details className="round-controls-tile round-collapsible-tile"><summary>Round controls</summary><div className="round-collapsible-content">{session.isHost && <><Toggle id="auto-advance" labelText="Auto-advance speech" labelA="Off" labelB="On" toggled={session.autoAdvance === true} onToggle={(toggled) => bridge.dispatch("session.setAutoAdvance", { enabled: toggled })} /><Button kind="tertiary" onClick={() => bridge.dispatch("session.advanceSpeech", { direction: "previous" })}>Previous speech</Button><Button kind="tertiary" onClick={() => bridge.dispatch("session.advanceSpeech", { direction: "next" })}>Next speech</Button><Button kind="ghost" onClick={() => bridge.dispatch("timer.resetAll")}>Reset timers</Button><div className="winner-actions"><Button kind="danger--tertiary" onClick={() => setWinner("affirmative")}>Affirmative won</Button><Button kind="danger--tertiary" onClick={() => setWinner("negative")}>Negative won</Button></div></>}</div></details></aside>
        </div>
      )}
      <Modal open={Boolean(winner)} danger modalHeading="Save round result" primaryButtonText="Save result" secondaryButtonText="Cancel" onRequestClose={() => setWinner(undefined)} onRequestSubmit={() => { if (winner) bridge.dispatch("session.saveRound", { winner }); setWinner(undefined); }}><p>Save this round as an {winner} win?</p></Modal>
    </>
  );
}

function SpeechOrder({ session, bridge }: { session: NonNullable<Snapshot["session"]>; bridge: EngineBridge }) {
  const listRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    listRef.current?.querySelector<HTMLElement>(".speech-row.is-current")?.scrollIntoView({ block: "nearest" });
  }, [session.currentSpeechIndex]);
  return <Tile className="speech-order-tile"><div className="tile-kicker">Speech order</div><div className="speech-list" ref={listRef}>{session.speechOrder.map((speech, index) => <button className={`speech-row ${index === session.currentSpeechIndex ? "is-current" : ""}`} key={speech.id} disabled={!session.isHost} onClick={() => bridge.dispatch("session.selectSpeech", { index })}><span>{speech.label}</span><span>{Math.round(speech.durationMs / 60000)} min</span></button>)}</div></Tile>;
}
...
function AiPage({ snapshot, bridge }: { snapshot: Snapshot; bridge: EngineBridge }) {
  ...
  const [chatMenu, setChatMenu] = useState<{ id: string; x: number; y: number }>();
  ...
  const citationDocuments = [...snapshot.documents, ...snapshot.cards.map((card) => ({ id: card.id, name: card.title, content: card.text, sourceType: "evidence" }))];
  return <><PageHeader title="AI Coach" /><div className="ai-layout"><aside className="chat-list"><div className="chat-section-title chat-section-heading"><span>Chats</span><Button kind="ghost" size="sm" renderIcon={Add} onClick={() => bridge.dispatch("ai.newChat")}>New chat</Button></div>{snapshot.ai.chats.length === 0 && <p className="muted-copy">Start a chat to practice an argument.</p>}{snapshot.ai.chats.map((item) => <div className={`chat-row ${item.id === chat?.id ? "is-selected" : ""}`} key={item.id} onContextMenu={(event) => openChatMenu(event, item.id)}><button className="chat-item" onClick={() => bridge.dispatch("ai.selectChat", { id: item.id })}>{item.title}</button><button type="button" className="document-menu-trigger" aria-label={`Actions for ${item.title}`} onClick={(event) => openChatMenu(event, item.id)}><OverflowMenuVertical /></button></div>)}</aside><section className="chat-panel"><div className="chat-messages">{chat?.messages.length ? chat.messages.map((item, index) => <div className={`message ${item.role}`} key={`${item.timestamp}-${index}`}><span className="message-role">{item.role === "user" ? "You" : "AI Coach"}</span><p>{item.text}</p></div>) : <EmptyState icon={Chat} title="Start a coaching chat" description="Choose approved document context, then send a prompt." />}{snapshot.ai.loading && <InlineNotification kind="info" lowContrast title="AI Coach is thinking" hideCloseButton />}</div><div className="chat-composer"><TextArea id="ai-message" labelText="Message" hideLabel placeholder="Ask for a rebuttal drill..." value={message} onChange={(event) => setMessage(event.currentTarget.value)} rows={3} /><div className="chat-composer-actions"><CiteMenu documents={citationDocuments} cited={cited} onToggle={(id, selected) => bridge.dispatch("ai.toggleCitation", { id, selected })} /><Button disabled={snapshot.ai.loading || !chat} onClick={() => { if (!message.trim()) return; bridge.dispatch("ai.sendMessage", { text: message.trim() }); setMessage(""); }}>Send</Button></div></div></section></div>{chatMenu && <WorkspaceContextMenu x={chatMenu.x} y={chatMenu.y} actions={[{ label: "Rename", onClick: () => { const item = snapshot.ai.chats.find((candidate) => candidate.id === chatMenu.id); if (item) startRenameChat(item); } }, { label: "Delete", danger: true, onClick: () => { const item = snapshot.ai.chats.find((candidate) => candidate.id === chatMenu.id); setChatMenu(undefined); if (item) setDeleteChat(item); } }]} />}{<Modal open={Boolean(renameChat)} modalHeading="Rename chat" primaryButtonText="Rename" primaryButtonDisabled={!renameValue.trim()} secondaryButtonText="Cancel" onRequestClose={() => setRenameChat(undefined)} onRequestSubmit={() => { if (renameChat && renameValue.trim()) bridge.dispatch("ai.renameChat", { id: renameChat.id, title: renameValue.trim() }); setRenameChat(undefined); }}><TextInput id="rename-chat" labelText="Chat name" value={renameValue} onChange={(event) => setRenameValue(event.currentTarget.value)} autoFocus /></Modal>}<Modal open={Boolean(deleteChat)} danger modalHeading="Delete chat" primaryButtonText="Delete" secondaryButtonText="Cancel" onRequestClose={() => setDeleteChat(undefined)} onRequestSubmit={() => { if (deleteChat) bridge.dispatch("ai.deleteChat", { id: deleteChat.id }); setDeleteChat(undefined); }}><p>Delete {deleteChat?.title ?? "this chat"}?</p></Modal></>;
}
