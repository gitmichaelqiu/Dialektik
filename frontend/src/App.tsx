...
function CiteMenu({ documents, cited, onToggle }: { documents: DebateDocument[]; cited: Set<string>; onToggle: (id: string, selected: boolean) => void }) {
  ...
  return <><Button className="cite-menu-trigger" kind="ghost" size="sm" onClick={toggle}>Cite{cited.size ? ` (${cited.size})` : ""}</Button><CarbonMenu open={open} label="Cite" x={position.x} y={position.y} onClose={() => setOpen(false)}>{documents.length === 0 ? <MenuItem disabled label="No documents available" /> : documents.map((document) => { const unavailable = !document.content.trim(); return <MenuItem key={document.id} disabled={unavailable} label={`${cited.has(document.id) ? "✓ " : ""}${document.name}${unavailable ? " (empty)" : ""}`} onClick={() => { onToggle(document.id, !cited.has(document.id)); setOpen(true); }} />; })}</CarbonMenu></>;
}
...
function AiPage({ snapshot, bridge }: { snapshot: Snapshot; bridge: EngineBridge }) {
  ...
  const citationDocuments = snapshot.documents;
