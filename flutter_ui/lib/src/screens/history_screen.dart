...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final affWinRate =
        affs.isNotEmpty ? (affWins / affs.length * 100).round() : 0;
    final negWinRate =
        negs.isNotEmpty ? (negWins / negs.length * 100).round() : 0;
    ...
    Widget buildDetailsPane(HistoryRecord record, bool insideDialog) {
      ...
      final dateStr = DateTime.fromMillisecondsSinceEpoch(record.timestamp)
          .toLocal()
          .toString()
          .split(' ')[0];
            ...
            children: [
                        ...
                        Wrap(
                          ...
                          children: [
                            Text('Date: $dateStr',
                                style: Theme.of(context).textTheme.bodySmall),
                            Text(
                                'Opponent: ${record.opponentName.isEmpty ? "Unknown" : record.opponentName}',
                                style: Theme.of(context).textTheme.bodySmall),
                            Text('Side: ${record.side}',
                                style: Theme.of(context).textTheme.bodySmall),
                          ],
                        ),
                      ],
                  ...
                  OutlinedButton(
                    onPressed: () async {
                      ...
                      final confirm = await _confirmAction(
                        ...
                        title: 'Delete Log',
                        content:
                            'Are you sure you want to delete this round log from history? This action cannot be undone.',
                      );
                      if (confirm && mounted) {
                        widget.bridge.dispatch(
                            action('history.delete', {'id': record.id}));
                        setState(() {
              ...
              Expanded(
                child: record.flows.isEmpty
                    ? const Center(
                        child: Text('No notes logged for this speech.'))
                    : GridView.builder(
                        gridDelegate:
            ...
            SectionHeader(
              ...
              trailing: Wrap(
                children: [
                  IconButton(
                    onPressed: () =>
                        widget.bridge.dispatch(action('history.refresh')),
                  ...
                  IconButton(
                    onPressed: () {
                      final historyJson = records
                          ...
                          .toList();
                      Clipboard.setData(
                          ClipboardData(text: jsonEncode(historyJson)));
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text(
                                'History exported and copied to clipboard!')),
                      );
                    },
